...
//TODO Pull some of these copied methods into the router
...
const AccountSettings = React.createClass({
  ...
  formElements() {
    ...
    return {
      ...
      email_notification: {
        ...
        errorClass: '',
        value: profile.email_notification || '',
        validator: (value) => {
          const valid = FormHelpers.checks.isRequired(value);
          const { formElements } = this.state;
          if (!valid) {
        ...
        update: (value) => {
          const { profile } = this.state;
          profile.email_notification = value;
          this.setState({ profile });
        }
      },
    }
  ...
  render() {
    const { formElements, handleChange } = this.states;
