...
def notify_user(object, action, site):
    ...
    if action == 'rejection':
        ...
        return
    template_content = [
        {'name': 'sender', 'content': object.obfuscated_sender},
        {'name': 'recipient', 'content': object.obfuscated_recipient},
    ]
    template_name = None
    if parent:
        if parent.sender_id == object.recipient_id:
            template_name = 'new-response'
    else:
