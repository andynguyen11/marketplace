...
def get_user_representation(user):
    ...
    """
    if settings.POSTMAN_SHOW_USER_AS == 'profile':
...
class MessageManager(models.Manager):
    ...
    def bookmarks(self, user, **kwargs):
        """
        Return messages belonging to a user and marked as bookmarked.
        """
        related = ('sender', 'recipient')
        filters = ({
            'recipient': user,
            'recipient_bookmarked': True,
            'recipient_deleted_at__isnull': True,
            'moderation_status': STATUS_ACCEPTED,
        }, {
            'sender': user,
            'sender_bookmarked': True,
            'sender_deleted_at__isnull': True,
        })
        return self._folder(related, filters, **kwargs)
