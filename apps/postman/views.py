...
from profiles.views import _user_profile
from profiles.models import Quote, Inquiry
...
class NamespaceMixin(object):
    ...
    def render_to_response(self, context, **response_kwargs):
        user_profile = _user_profile(self.request)
...
class BookmarksView(FolderMixin, TemplateView):
    """
    Display the list of archived messages for the current user.
    ...
    view_name = 'bookmarks'
    # for TemplateView:
    template_name = 'postman/bookmarks.html'
...
class DisplayMixin(NamespaceMixin, object):
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            quote = Quote.objects.get(message=self.msgs[0])
            context['quote'] = quote
        except Quote.DoesNotExist:
            pass
        try:
            inquiry = Inquiry.objects.get(message=self.msgs[0])
...
class BookmarkView(UpdateMessageMixin, View):
    """Mark messages/conversations as bookmarked."""
    field_bit = 'bookmarked'
    success_msg = ugettext_lazy("Messages or conversations successfully bookmarked.")
    field_value = True


class UnbookmarkView(UpdateMessageMixin, View):
    """Mark messages/conversations as unbookmarked."""
    field_bit = 'bookmarked'
    success_msg = ugettext_lazy("Messages or conversations successfully unbookmarked.")
    field_value = False
...
class UnarchiveView(UpdateMessageMixin, View):
    """Mark messages/conversations as archived."""
    field_bit = 'archived'
    success_msg = ugettext_lazy("Messages or conversations successfully unarchived.")
    field_value = False
